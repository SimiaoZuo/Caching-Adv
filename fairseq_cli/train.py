...
from adv_dataset import AdvSamples
from fairseq import (
    checkpoint_utils,
    distributed_utils,
    options,
...
def main(args, init_distributed=False, adv_samples=None):
    ...
    extra_state, epoch_itr = checkpoint_utils.load_checkpoint(args, trainer, adv_samples=adv_samples)
    if args.tpu:
...
def distributed_main(i, args, start_rank=0, adv_samples=None):
    args.device_id = i
    if args.distributed_rank is None:  # torch.multiprocessing.spawn
        args.distributed_rank = start_rank + i
    main(args, init_distributed=True, adv_samples=adv_samples)
...
def cli_main_helper(args):
    ...
    logger.info('loading neighbor information')
    task = tasks.setup_task(args)  # we need to get the training dataset sizes
    task.load_dataset('train', combine=False, epoch=1)
    if args.mode == 'cache':
        adv_samples = AdvSamples(args, task.datasets['train'].src_sizes, task.datasets['train'].tgt_sizes)
    else:
        adv_samples = None
    del task
    ...
    if args.distributed_init_method is not None:
        ...
        if not getattr(args, "tpu", False):
            ...
            torch.multiprocessing.spawn(
                fn=distributed_main, args=(args, 0, adv_samples), nprocs=args.distributed_world_size
            )
        else:
        ...
        # single GPU training
        main(args, adv_samples=adv_samples)
